...
resource "aws_instance" "wp-app-server" {
  ...
  user_data = <<-EOF
    ...
    sudo apt update
    sudo apt -y install docker-ce docker-ce-cli containerd.io docker-compose-plugin git
    sudo usermod -aG docker ubuntu

    git clone https://github.com/OrlenkoAndriy/cloud-demo.git

    cd /cloud-demo/app && docker compose up -d
  EOF
